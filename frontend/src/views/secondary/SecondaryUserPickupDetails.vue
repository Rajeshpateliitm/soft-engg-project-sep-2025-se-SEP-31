<template>
  <div class="secondary-pickup-details">
    <div class="container">
      <div class="row mb-4">
        <div class="col-12">
          <h2 class="text-success fw-bold mb-3">DAILY WASTE COLLECTION LOG</h2>
          <p class="text-50">
            View daily waste logs from households in your RWA area
          </p>
        </div>
      </div>
          ...
          <div class="card shadow-lg">
            <div class="card-body">
              <div class="row align-items-center">
                <div class="col-md-6">
                  <label for="wasteLogDate" class="form-label fw-semibold"
                    >Select Date to View Waste Logs</label
                  >
                  <input
                    type="date"
                    id="wasteLogDate"
          ...
          <div class="card shadow-lg h-100 border-primary">
            <div class="card-body text-center">
              ...
              <h3 class="card-text fw-bold text-primary">
                {{ wasteSummary.total_households }}
              </h3>
              <small class="text-muted"
                >{{ wasteSummary.households_logged }} logged today</small
              >
            </div>
          </div>
        </div>
        ...
        <div class="col-md-3 mb-3">
          <div class="card shadow-lg h-100 border-info">
            <div class="card-body text-center">
              ...
              <h3 class="card-text fw-bold text-info">
                {{ wasteSummary.households_separated }}
              </h3>
          ...
          <div class="card shadow-lg h-100 border-warning">
            <div class="card-body text-center">
              ...
              <h3 class="card-text fw-bold text-warning">
                {{ wasteSummary.households_recycled }}
              </h3>
              <small class="text-muted">Households</small>
            </div>
          </div>
          ...
          <div class="card shadow-lg border-success">
            <div class="card-body">
              ...
              <h4 class="card-text fw-bold text-success">
                {{ wasteSummary.total_wet_waste }} KG
              </h4>
            </div>
          </div>
        </div>
          ...
          <div class="card shadow-lg border-info">
            <div class="card-body">
              ...
              <h4 class="card-text fw-bold text-info">
                {{ wasteSummary.total_dry_waste }} KG
              </h4>
            </div>
          </div>
        </div>
        <div class="col-md-4 mb-3">
          <div class="card shadow-lg border-danger">
            <div class="card-body">
              ...
              <h4 class="card-text fw-bold text-danger">
                {{ wasteSummary.total_hazardous_waste }} KG
              </h4>
            </div>
          </div>
        </div>
          ...
          <div class="card shadow-lg">
            ...
            >
              <h5 class="card-title mb-0">
                HOUSEHOLD WASTE LOGS FOR {{ formattedDate }}
              </h5>
            ...
            <div class="card-body">
              ...
              <div
                v-else-if="householdWaste.length === 0"
              ...
              <div v-else class="table-responsive">
                <table class="table table-hover">
                  ...
                  <tbody>
                    <tr
                      v-for="household in householdWaste"
                      :key="household.user_id"
                      :class="{ 'table-warning': !household.has_logged }"
                    >
                      <td>
                        <strong>{{ household.house_number || "N/A" }}</strong>
                      </td>
                      ...
                      <td>
                        <span
                          ...
                          >{{ household.wet_waste.toFixed(2) }}</span
                        >
                      ...
                      <td>
                        <span
                          ...
                          >{{ household.dry_waste.toFixed(2) }}</span
                        >
                        <span v-else class="text-muted">-</span>
                      </td>
                      <td>
                        <span
                          ...
                          >{{ household.hazardous_waste.toFixed(2) }}</span
                        >
                        <span v-else class="text-muted">-</span>
                      </td>
                      <td>
                        <strong v-if="household.has_logged">{{
                          household.total_waste.toFixed(2)
                        }}</strong>
                      ...
                      <td>
                        ...
                        <span
                          v-if="household.separated"
                          class="badge bg-info ms-1"
                          title="Waste was properly separated"
                        >
                          <i class="bi bi-check2"></i>Separated
                        </span>
                        <span
                          v-if="household.recycled"
                          class="badge bg-success ms-1"
                          title="Waste was recycled/reused"
                        >
                  ...
                  <tfoot v-if="householdWaste.length > 0" class="table-active">
                    <tr>
                      ...
                      <td class="fw-bold">
                        {{ totalHazardousWaste.toFixed(2) }} KG
                      </td>
...
const totalHazardousWaste = computed(() => {
  return householdWaste.value.reduce(
    (sum, h) => sum + (h.hazardous_waste || 0),
    0
  );
});
