...
def handle_api_error(error):
    ...
    if isinstance(error, ValueError):
        current_app.logger.error(f"Validation error: {str(error)}")
...
def chat(user):
    ...
    try:
        ...
        if not ai_response:
            ...
            if "candidates" in response_data and len(response_data["candidates"]) > 0:
                ...
                if candidate.get("finishReason") != "STOP":
                    ...
                    raise ValueError(f"Response blocked by API")
        ...
        # Return only user-friendly message, no technical details
...
def dashboard_analysis(user):
    ...
    try:
        ...
        if not gemini_key:
            current_app.logger.error("Gemini API key not configured for dashboard analysis")
            return jsonify({"error": "The service is temporarily unavailable. Please try again later."}), 500
...
Do NOT use ```html or ``` tags. Keep it compact and professional."""
        ...
        if not analysis_text:
            current_app.logger.error("Empty response from API for dashboard analysis")
